...
```
  ...
  +--------------------------------|-----------------------------------------+
                                   |  fetch, cookies em_session + em_csrf
                                   |  dev: Vite proxy /api -> 127.0.0.1:8000
                                   |  prod: FastAPI StaticFiles serves the SPA
  +--------------------------------v-----------------------------------------+
  |  FastAPI process (uvicorn, workers=1)                                     |
  |                                                                           |
...
| File | Purpose |
|---|---|
| `vite.config.ts` | React and Tailwind plugins, `@` alias, and the `/api` proxy to 127.0.0.1:8000 with `changeOrigin: false`. |
...
There is exactly one browser origin in development (Vite proxy) and in production (FastAPI
StaticFiles), so `CORSMiddleware` is never added to this codebase at all.
...
```
terminal 1:  cd backend  && uv run expirymanager --reload
...
cd backend  && uv run expirymanager   # serves the API and dist from 127.0.0.1:8000
```
